...
SandCastle.prototype.kickOverSandCastle = function() {
  ...
  if (this.sandboxInitialized) {
    ...
    // try to delete the shared mmap after a kick
    if (this.spawnFile) {
      try{ fs.unlinkSync(`/tmp/shared-${this.spawnFile}`); } catch (e) {} 
    }
  }
};
...
SandCastle.prototype.spawnSandbox = function() {
  ...
  this.sandbox.stdout.on('data', (data) => {
    data = data.toString();
    if (data.indexOf('sandbox file') === 0) {
      this.spawnFile = data.substring(data.indexOf(': ')+2);
    }
